...
{
public:
    int digitalRead(gpio_num_t pin);              // read gpio pin
    void digitalWrite(gpio_num_t pin, int value); // write in gpio pin the value
    void pinMode(gpio_num_t pin, int config);     // config pin mode (input/output/pullup)
};
